...
cibar target_sanctioncost if period < 10, ///
	...
	ciopts(lcolor(black)) ///
	graphopts( ///
		legend(cols(3)) ytitle("Average sanction sent") ylabel(0(0.5)3,nogrid) ///
		subtitle("{bf:A}", ring(0) pos(10) size(large)) ///
		name(senders_exog, replace) nodraw)
cibar target_sanctioncost if period < 10, ///
	over1(target_type) over2(endow_observe) ///
	barcolor(gray*0.33 gray*0.66 gray) ///
	ciopts(lcolor(black)) ///
	graphopts( ///
		ytitle("Average sanction received") ylabel(0(0.5)3,nogrid) ///
		subtitle("{bf:B}", ring(0) pos(10) size(large)) ///
		name(receivers_exog, replace) nodraw)		
restore
grc1leg total_sanctions_sent total_sanctions_received senders_exog receivers_exog, ycommon legendfrom(senders_exog) 
